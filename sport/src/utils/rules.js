...
export function checkNumPot2(rule,value,callback){
    ...
    if (!value) {
      ...
      return callback(new Error('请填写数字,最多 2 位小数'))
    } else {
      callback()
    }
}
...
export function checkIdNum(rule, value, callback) {
  ...
  export function validateTelphone(rule, value,callback) {
    ...
    if(value==''||value==undefined||value==null){
      ...
      if ((!reg.test(value)) && value != '') {
        callback(new Error('请输入正确的固话（格式：区号+号码,如 010-1234567）'));
      } else {
        callback();
      }
    }
  ...
  export function isEnglish(rule, value, callback) {
    ...
    if(value==''||value==undefined||value==null||value=='******'){
      ...
      if (!reg.test(value)){
        callback(new Error('只允许输入英文字母、数字以及下划线'));
      } else {
        callback();
      }
    }
  ...
  // 验证端口是否在[0,65535]之间，非必填,isMust 表示是否必填
